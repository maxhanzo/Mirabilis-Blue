...
protocol FileTransferServiceObserving: AnyObject {
...
private final class WeakFileTransferServiceObserver {

    weak var value: FileTransferServiceObserving?

    init(
        _ value: FileTransferServiceObserving
    ) {
        self.value = value
    }
}
...
final class FileTransferService {
    ...
    private var observers:
        [WeakFileTransferServiceObserver] = []
...
extension FileTransferService {
    ...
    func addObserver(
        _ observer: FileTransferServiceObserving
    ) {
        cleanupObservers()

        guard !observers.contains(
            where: { $0.value === observer }
        ) else {
            return
        }

        observers.append(
            WeakFileTransferServiceObserver(observer)
        )
    }
...
extension FileTransferService: BluetoothObserving {
    ...
    func bluetoothManager(
        ...
        case .notificationStateChanged(
            ...
            handleFileTransferTXNotificationState(
                isEnabled: isEnabled
            )
